...
function createOrgTable() {
	$myTable.datagrid({
		...
		striped : true,
		url : $ctx + '/orgMng/listOrg.do?orgBtype=wlagent', // controller地址
...
function addOrg() {
	$("#save-button").show();
    $("#update-button").hide();
var url = $ctx + '/pages/sale/agentmng/addAgent.jsp';
	$("#orgDialog").dialog({
		title:"新增网络代理商",
		resizable:true,
		inline:true,
...
function updateOrg(index) {
	...
	var rowData = $myTable.datagrid("getRows")[index];
	var orgId = rowData.orgId;
	var url = $ctx + '/orgMng/beforeUpdteOrg.do?orgId='+orgId+'&orgBtype=wlagent';//网络代理商
		$("#orgDialog").dialog({
			title:"修改网络代理商",
			resizable:true,
			inline:true,
...
function updateToken() {
	...
	if (selRow.length == 0) {
		...
		$.messager.confirm('提示', '您确定要重置选中的' + '[' + selRow.length
				+ ']个网络代理商的授权码吗?', function(r) {
			if (r) {
				$.ajax({
