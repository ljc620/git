...
import com.zhming.support.util.AnnotationUtil;
...
public class DBUtilImpl implements DBUtil {
	...
	@Override
	public <T> PageBean<T> queryPageToBean(String prefix, String sqlStr, Class<T> beanClass, int page, int pageSize, Object... params) {
		Session session = getSession();
		...
		try {
			...
			if (pageSize == 0) {// 不分页
				...
				pageBean = new PageBean<T>(page, pageSize, retList.size(), retList);
				return pageBean;
			}

			int totalNum = this.count(prefix, sqlStr, params);
			...
			pageBean = new PageBean<T>(page, pageSize, totalNum, result);
	...
	@Override
	public PageBean<Map<String, Object>> queryPageToMap(String prefix, String sqlStr, int page, int pageSize, Object... params) {
		Session session = getSession();
		...
		try {
			...
			if (pageSize == 0) {// 不分页
				List<Map<String, Object>> retList = queryListToMap(prefix, sqlStr, params);
				pageBean = new PageBean<Map<String, Object>>(page, pageSize, retList.size(), retList);
				return pageBean;
			}
			int totalNum = this.count(prefix, sqlStr, params);
			...
			pageBean = new PageBean<Map<String, Object>>(page, pageSize, totalNum, result);
	...
	@Override
	public <T> Map<String, T> queryMapForBean(String prefix, String sqlStr, Class<T> beanClass, String fieldName, Object... params) throws DBException {
		List<T> retList = queryListToBean(prefix, sqlStr, beanClass, params);
		...
		for (T obj : retList) {
			...
			try {
				...
				throw new DBException(3, "获取bean值出错");
			}
			retMap.put(key, obj);
		}
		return retMap;
	...
	/*
	 * protected <T> void setScalar(SQLQuery query, Class<T> beanClass) { List<FieldBean> fields = BeanUtil.getBeanField(beanClass); for (FieldBean field : fields) { if
	 * (field.getFieldType().equals("java.lang.String")) { query.addScalar(field.getFieldName(), StringType.INSTANCE); } else { query.addScalar(field.getFieldName()); } } }
	 */
