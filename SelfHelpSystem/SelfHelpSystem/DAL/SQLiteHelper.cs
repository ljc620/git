...
namespace SelfHelpSystem.DAL
{
    public  class SQLiteHelper
    {
        ...
        public static string LocalDbConnectionString = "Data Source=" + AppDomain.CurrentDomain.BaseDirectory + "Data\\CSData.db;Version=3;";//;Version=3;
        ...
        public static int ExecuteNonQuery_Test(string connectionString, SQLiteCommand cmd)
        {
            int result = 0;
            if (connectionString == null || connectionString.Length == 0)
                throw new ArgumentNullException("connectionString");
            using (SQLiteConnection con = new SQLiteConnection(connectionString))
            {
                PrepareCommand_Test(cmd, con,cmd.CommandType, cmd.CommandText);
                try
                {
                    result = cmd.ExecuteNonQuery();
                }
        ...
        #region  预处理Command对象,数据库连接,事务,需要执行的对象,参数等的初始化
        /// <summary>  
        /// 预处理Command对象,数据库连接,事务,需要执行的对象,参数等的初始化  
        /// </summary>  
        ...
        public static void PrepareCommand_Test(SQLiteCommand cmd, SQLiteConnection conn, CommandType cmdType, string cmdText, params SQLiteParameter[] cmdParms)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            cmd.Connection = conn;
