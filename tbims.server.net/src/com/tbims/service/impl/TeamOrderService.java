...
public class TeamOrderService extends BaseService implements ITeamOrderService {
	...
	private void doCommit(Session session, SlOrg slOrg, SlTeamOrder slTeamOrder, List<SlTeamOrderDetail> detailList) throws ServiceException, DBException {
		...
		Map<String, Object> mapParemeter = dbUtil.queryFirstForMap("查询签约社最低预付款限额", "select PAREMETER_VAL from SYS_PAREMETER where PAREMETER_ID='team_advance_limit'");
		
		int teamAdvanceLimit=0;
		if(mapParemeter.containsKey("paremeterVal") && !StringUtil.convertString(mapParemeter.get("paremeterVal")).equals(""))
			teamAdvanceLimit = Integer.valueOf(StringUtil.convertString(mapParemeter.get("paremeterVal")));
		...
		for (SlTeamOrderDetail slTeamOrderDetail : detailList) {
			...
			if (slLimitAmt == null) {
				// 如果额度为空，判断预付款
				if (slOrg.getAdvanceAmt() <= teamAdvanceLimit) {
					// 如果预付款为0
					...
					// Long frozenAdvanceAmt = slTeamOrderDetail.getApplyNum() * price;
					// 判断预付款余额是否充足
					if (slOrg.getAdvanceAmt() - teamAdvanceLimit < frozenAdvanceAmt) {
						// 如果预付款余额不足
						throw new ServiceException(MSG.BF_ERROR_AMT);
					} else {
						// 预付款余额充足，冻结预付款
				...
				if (limitAmt >= slTeamOrderDetail.getApplyNum()) {
					...
					// 判断预付款是否充足
					if (slOrg.getAdvanceAmt() - teamAdvanceLimit >= remainAavance) {
						// 如果充足，扣减额度及预付款
	...
	public PageBean<TeamOrderBean> listApplyHis(UserBean userBean, TeamOrderBean teamOrderBean) throws Exception {
		...
		if (teamOrderBean != null) {
			if (StringUtil.isNotNull(teamOrderBean.getStat())) {
				...
				sb.append(" AND T.STAT in('04','05','06','07') ");
			}
			if (StringUtil.isNotNull(teamOrderBean.getApplyId())) {
				sb.append(" AND T.APPLY_ID=:APPLY_ID ");
				params.put("APPLY_ID", teamOrderBean.getApplyId());
	...
	public void addOrder(UserBean userBean, SlTeamOrder slTeamOrder, List<SlTeamOrderDetail> detailList) throws Exception {
		...
		try {
			...
			//SlOrg slOrg = dbUtil.findById("", SlOrg.class, userBean.getOrgId());
			SlOrg slOrg=dbUtil.queryFirstListToBeanLock("查询机构信息", session, "FROM SlOrg U where orgId=?", "U", SlOrg.class, userBean.getOrgId());

			Date inDt = slTeamOrder.getInDt();
			String currDateStr = DateUtil.formatDateToString(new Date(), "yyyy-MM-dd");
			if (inDt.getTime() < (DateUtil.addDay(DateUtil.formatStringToDate(currDateStr, "yyyy-MM-dd"), Integer.valueOf(getParemeterVal("before_apply_day")))).getTime()) {
				throw new ServiceException(MSG.ERROR, "入园日期应大于等于当前日期" + getParemeterVal("before_apply_day") + "天！");
			}
			...
			for (SlTeamOrderDetail slTeamOrderDetail : detailList) {
				SysTicketTypePrice sysTicketTypePriceNew = dbUtil.queryFirstForBean("查询是否超过三个阶梯票价段", "SELECT * FROM SYS_TICKET_TYPE_PRICE T WHERE T.TICKET_TYPE_ID=? AND ?>END_NO-START_NO+1",
						SysTicketTypePrice.class, slTeamOrderDetail.getTicketTypeId(), slTeamOrderDetail.getApplyNum());
				if (sysTicketTypePriceNew != null) {
			...
			// 临时保存
			if (Constant.TEAM_ORDER_STAT_01.equals(slTeamOrder.getStat())) {
				List<SlTeamOrderDetail> newDetailList = new ArrayList<SlTeamOrderDetail>();
				Long totalNum = 0l;
				for (SlTeamOrderDetail slTeamOrderDetail : detailList) {
					totalNum = totalNum + slTeamOrderDetail.getApplyNum();
			...
			// 正式保存，需冻结额度及预付款
				...
				doCommit(session, slOrg, slTeamOrder, detailList);
			}
	...
	public void commitOrder(String applyId) throws Exception {
		...
		try {
			...
			//SlOrg slOrg = dbUtil.findById("", SlOrg.class, slTeamOrder.getOrgId());
			...
			slTeamOrder.setStat(Constant.TEAM_ORDER_STAT_02);
			doCommit(session, slOrg, slTeamOrder, slTeamOrder.getSlTeamOrderDetails());
			dbUtil.commit(session);
		} finally {
			dbUtil.close(session);
		}
	...
	public void updChangeUser(TeamOrderBean teamOrderBean) throws ServiceException {
		...
		SlChangeUser slChangeUser = dbUtil.findById("", SlChangeUser.class, teamOrderBean.getChangeUserId());
	...
	public void cancleOrder(UserBean userBean, SlTeamOrder slTeamOrder) throws Exception {
		...
		if (!Constant.TEAM_ORDER_STAT_02.equals(slTeamOrderNew.getStat())) {
		...
		slTeamOrderNew.setStat(Constant.TEAM_ORDER_STAT_07);
