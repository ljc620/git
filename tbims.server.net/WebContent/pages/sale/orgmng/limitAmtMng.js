...
function getStr(value, row, index) {
	var s1;
	s1 = '<a href="javascript:void(0)" title="额度明细"  onclick="getLimitDetail(' + index + ')">额度明细</a> ';
	return s1;
}
...
function addLimit(obj) {
	$("#save-button").show();
var url = $ctx + '/orgMng/getOrg.do?orgId='+obj+'&flag=o';
...
function saveLimit() {
	...
	$.messager.confirm('提示', '确定保存信息吗?', function(r) {
		if (r) {
			...
			$.ajax({
				url : $ctx + "/orgMng/addLimit.do?flag=o",
				type : "post",
				data : $("#fm").serialize(),
				success : function(e) {
					$.messager.show({
