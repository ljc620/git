...
function getstr(value, row, index) {
	...
	if(rowData.stat=='02'){
		s2 = '<a href="javascript:void(0)" title="取消订单" class="opr-btn icon-stop" onclick="cancleExam(' + index + ')"></a> ';
	}
	if(rowData.stat=='01'){
...
function commitOrder(index){
	var rowData = $myTable.datagrid("getRows")[index];
	var applyId = rowData.applyId;
	$.messager.confirm('提示', '提交后不可删除，您确定要提交[' + rowData.applyId + ']记录吗?', function(r) {
		if (r) {
			$.ajax({
...
function cancleExam(index){
	...
	$.messager.confirm('提示', '您确定要取消订单号为：[' + rowData.applyId + ']的订单吗?', function(r) {
		if (r) {
			$.ajax({
				url : $ctx + "/teamOrder/cancleOrder.do",
				type : "post",
				data : {applyId:applyId},
				success : function(e) {
					$.messager.show({
